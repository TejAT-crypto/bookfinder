...
const UserLogin = ({ socket }) => {
  ...
  const submit = async (data) => {
    axios
      .post("http://10.1.123.86:3000/auth/login", data)
      .then((res) => {
        console.log(res);
    ...
    <section className="relative h-screen bg-cover bg-center">
      ...
      <div className="relative w-full h-full flex flex-row mx-auto md:h-screen lg:py-0 animate-slide-in-2 overflow-hidden z-0">
        ...
        <div className="relative flex-shrink-0 w-full md:w-1/2 overflow-hidden">
          <div
            className="relative backdrop-blur-sm flex h-screen items-center justify-center bg-black bg-opacity-25 rounded-3xl md:rounded-tl-3xl md:rounded-bl-3xl md:rounded-r-none overflow-hidden"
            style={{
              backgroundImage:
                ...
                > 
                  <div className='pt-1.5'>
                    <input
                      ...
                      name="email"
                      id="email"
                      className="items-center justify-center border-b-2 border-[#FFF5E0] bg-transparent text-[#FFF5E0] placeholder-[#FFF5E0] text-xs md:text-xl sm:text-base w-10/12 pb-0 md:pb-1 outline-none"
                      placeholder="E-mail"
                      {...register("email")}
                      required
                    />
                  ...
                  <div className='pt-1.5 pb-2.5'>
                    <input
                      ...
                      className="items-center justify-center border-b-2 border-[#FFF5E0] bg-transparent text-[#FFF5E0] placeholder-[#FFF5E0] md:text-xl text-xs sm:text-base w-10/12 pb-0 md:pb-1 outline-none"
                      {...register("password")}
                      required
                    />
                  </div>
                  <button
                    type="submit"
                    className="items-center justify-center bg-[#FFF5E0] md:text-xl text-xs sm:text-sm rounded-lg w-1/2 p-1 md:p-1.5 outline-none"
                  >
                    Log In
                  </button>
                </form>
